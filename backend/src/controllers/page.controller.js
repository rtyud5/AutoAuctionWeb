...
const reviewView = async (req, res) => {
  ...
  try {
    // Current score (align with negative-user rule: reputation = 10 + pos - neg)
    const [[uRow]] = await db.query(
      `SELECT positive_count, negative_count FROM users WHERE id = ? LIMIT 1`,
      { replacements: [userId], raw: true }
    );
    const positive = Number(uRow?.positive_count || 0);
    const negative = Number(uRow?.negative_count || 0);
    const net = 10 + positive - negative;
    const total = positive + negative;
    const percent = total > 0 ? Math.round((positive * 100) / total) : 0;

    const score = { positive, negative, net, percent };

    // Given ratings: user rated others
    ...
    // Received ratings: others rated user
    const [receivedRatings] = await db.query(
      ...
      SELECT
        ...
        r.comment,
        r.created_at,
        ru.name AS rater_name,
        p.title AS product_title
      FROM ratings r
      LEFT JOIN users ru ON ru.id = r.rater_id
      LEFT JOIN orders o ON o.id = r.order_id
      LEFT JOIN auctions a ON a.id = o.auction_id
      LEFT JOIN products p ON p.id = a.product_id
...
const itemHistoryView = async (req, res) => {
  ...
  try {
    ...
    // 1) All bids by user (manual + auto generated by engine)
    let bidRows = [];
    try {
      const [rows] = await db.query(
        `
      ...
      console.warn("itemHistoryView: skip bid rows:", e.message);
      bidRows = [];
    }

    // 2) Buy-now orders (to classify the final action correctly)
    const buyNowAuctionIds = new Set();
    const buyNowWinnerBidIds = new Set();
    try {
      const [rows] = await db.query(
        `
        SELECT
      ...
      (rows || []).forEach((r) => {
        buyNowAuctionIds.add(Number(r.auction_id));
        if (r.winner_bid_id) buyNowWinnerBidIds.add(Number(r.winner_bid_id));
        events.push({
          kind: "BUY_NOW",
          at: r.created_at,
      ...
      console.warn("itemHistoryView: skip buy-now rows:", e.message);
    }

    // 3) Convert bids to events (skip the winning bid that belongs to buy-now, to avoid duplicates)
    const lastAutoByAuction = new Map(); // auctionId -> last AUTO_BID amount (for marking update)
    bidRows.forEach((b) => {
      if (b?.id && buyNowWinnerBidIds.has(Number(b.id))) return;

      const auctionId = Number(b.auction_id);
      const isAuto = Number(b.is_auto) === 1;

      const ev = {
        kind: isAuto ? "AUTO_BID" : "BID",
        at: b.created_at,
      ...
      if (isAuto) {
        if (lastAutoByAuction.has(auctionId)) {
          ev.is_update = true;
        }
        lastAutoByAuction.set(auctionId, Number(b.amount || 0));
      }
      events.push(ev);
    });

    // 4) Win/Lose outcomes (only when auction ended)
    //    Include auctions where user participated (bid or auto-bid rule)
    try {
      const [rows] = await db.query(
        `
        SELECT
        ...
        WHERE a.end_time <= NOW()
          AND (a.status = 'ENDED' OR a.status = 'CANCELLED')
          AND (
            EXISTS (SELECT 1 FROM bids b WHERE b.auction_id = a.id AND b.bidder_id = ?)
            OR EXISTS (SELECT 1 FROM auto_bid_rules r WHERE r.auction_id = a.id AND r.bidder_id = ?)
          )
      ...
      (rows || []).forEach((r) => {
        const auctionId = Number(r.auction_id);
        const isWinner = Number(r.winner_id) === Number(userId);

        // If the auction ended by BUY_NOW of this user, showing BUY_NOW is enough.
        if (isWinner && buyNowAuctionIds.has(auctionId)) return;
      ...
      console.warn("itemHistoryView: skip outcome rows:", e.message);
    }
    ...
    events.sort((a, b) => {
      ...
      // Same time: show actions first, outcome after
      const rank = (k) => {
        switch (k) {
          case "BID":
          case "AUTO_BID":
...
// Watchlist state for current user (to render ★ correctly)
let isWatchlisted = false;
if (user && auction.id) {
  try {
    const [[w]] = await db.query(
      `SELECT 1 AS ok FROM watch_list WHERE user_id = ? AND auction_id = ? LIMIT 1`,
      { replacements: [user.id, auction.id], raw: true }
    );
    isWatchlisted = !!w;
  } catch (e) {
    isWatchlisted = false;
